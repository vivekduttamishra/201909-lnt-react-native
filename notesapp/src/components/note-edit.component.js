...
import { noteSave, noteSelect } from '../store/actions/action-creators';
...
class _Component extends React.Component {
    ...
    componentWillReceiveProps(props) {
        ...
        let id= props.mode===MODE_NEW? null: props.selectedNote.id;
        this.setState({ ...props.selectedNote,id });
        //console.log('after property, state', this.state);
    }
    save= ()=>{
        let note ={...this.state};

        if (this.props.mode==MODE_NEW){
            this.props.noteSave(null,note);
        } else{
            this.props.noteSave(this.state.id, note);
        }

        this.props.noteSelect(note);
...
export default connect(mapStateToProps,{noteSave,noteSelect})( _Component);
